...
`cryoet_catalog.db.init_schema(engine)` currently calls
...
4. From then on, every ORM change ships as a new revision: edit
   `cryoet_catalog/orm.py`, run `pixi run -e catalog migrate-revision -- "what changed"`,
...
(`tests/cryoet_catalog/test_orm_drift.py`) is what's keeping us honest in
the meantime.
