"""Unit tests for ``cryoet_catalog.parsers.*``.

Each test writes its own per-test fixtures inline using ``tmp_path`` rather
than depending on the shared ``tests/cryoet_catalog/fixtures/`` tree (which
...
from cryoet_catalog.parsers.toml_files import LoadResult, load_sample_toml
