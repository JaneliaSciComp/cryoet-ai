...
| 11.19 | URL filter scope | **Revised 2026-05-11 (Phase 7 review):** every drawer field round-trips through the URL — there is no "canonical subset" or local-only drawer state. The 300 ms debounce inside `FilterDrawer` stays and now also throttles URL writes (slider drags settle to one navigate per gesture instead of one per pixel). **Original decision:** minimal URL params (project/data_source/q/sort/order/limit/offset/selected id); drawer state in local React state initialized from search params on mount, **not pushed back**; "Copy filter URL" button as escape hatch. **Fetch-trigger debounce: 300 ms** on the drawer-state → query-key path so slider drags don't fire 60 requests/sec (§9.1) | Revised because user feedback during Phase 7 review preferred uniform behavior across all filters: back/forward, copy/paste, and bookmarks now affect every field the same way. Filter count is small enough that URL length isn't a concern. The 300 ms debounce alone prevents URL churn from slider drags. The "Copy filter URL" button becomes redundant (URL already has everything) — kept for now since it's a low-cost convenience. |
